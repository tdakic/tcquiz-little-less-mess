...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,_notification=_interopRequireDefault(_notification),_jquery=_interopRequireDefault(_jquery);const Selectors_regions={timeLeft:'[data-region="quizaccess_tcquiz/timeleft_span"]'},registerEventListeners=(sessionid,quizid,cmid,attemptid,page,time_for_question,POLLING_INTERVAL)=>{(0,_jquery.default)("#responseform").on("submit",(function(){(0,_jquery.default)("#responseformsubmit").attr("disabled","disabled")}));var timeLeft=time_for_question,timeLeft_html=document.querySelector(Selectors_regions.timeLeft),teacherEndedQuestion=!1,timer=setInterval((function(){timeLeft--,timeLeft_html.innerHTML=timeLeft,(timeLeft<=0||teacherEndedQuestion)&&(clearInterval(timer),clearInterval(tecaherEndedQuestionEvent),timer=null,timeLeft_html.innerHTML=0,window.goToCurrentQuizPageEvent=setInterval((async()=>{await async function(sessionid,quizid,cmid,attemptid){var result=await fetch(M.cfg.wwwroot+"/mod/quiz/accessrule/tcquiz/quizdatastudent.php?requesttype=getanswer&quizid="+quizid+"&sessionid="+sessionid+"&cmid="+cmid+"&attempt="+attemptid+"&sesskey="+M.cfg.sesskey,{method:"POST"}),response_xml_text=await result.text();await function(response_xml_text){const response_xml=(new DOMParser).parseFromString(response_xml_text,"text/html");var quizresponse=response_xml.getElementsByTagName("tcquiz").item(0);if(null===quizresponse)return void _notification.default.addNotification({message:(0,_str.get_string)("invalidserverresponse","quizaccess_tcquiz"),type:"error"});var quizstatus=quizresponse.getElementsByTagName("status").item(0).textContent;if("showquestion"==quizstatus);else if("showresults"==quizstatus){window.goToCurrentQuizPageEvent=null,clearInterval(window.goToCurrentQuizPageEvent);var result_url=quizresponse.getElementsByTagName("url").item(0).textContent;window.location.replace(result_url)}else if("finalresults"==quizstatus)window.goToCurrentQuizPageEvent=null,clearInterval(window.goToCurrentQuizPageEvent);else if("quiznotrunning"==quizstatus||"waitforquestion"==quizstatus||"waitforresults"==quizstatus||"noaction"==quizstatus);else if("error"==quizstatus){var errmsg=quizresponse.getElementsByTagName("message").item(0).textContent;_notification.default.addNotification({message:errmsg,type:"error"})}else _notification.default.addNotification({message:(0,_str.get_string)("unknownserverresponse","quizaccess_tcquiz")+quizstatus,type:"error"})}(response_xml_text)}(sessionid,quizid,cmid,attemptid)}),POLLING_INTERVAL))}),1e3);const tecaherEndedQuestionEvent=setInterval((async function(){teacherEndedQuestion=await async function(sessionid,quizid,cmid,attemptid){var result=await fetch(M.cfg.wwwroot+"/mod/quiz/accessrule/tcquiz/get_question_state.php?requesttype=getnumberanswers&quizid="+quizid+"&sessionid="+sessionid+"&cmid="+cmid+"&attempt="+attemptid+"&sesskey="+M.cfg.sesskey,{method:"POST"});return"0"==await result.text()}(sessionid,quizid,cmid,attemptid)}),POLLING_INTERVAL)};_exports.init=(sessionid,quizid,cmid,attemptid,page,time_for_question,POLLING_INTERVAL)=>{registerEventListeners(sessionid,quizid,cmid,attemptid,0,time_for_question,POLLING_INTERVAL)}}));
